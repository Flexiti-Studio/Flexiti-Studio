...
export default function ServicesTechStack() {
  ...
  return (
    <section className="py-24 px-6 md:px-8 overflow-hidden bg-slate-50 dark:bg-[#07070a] transition-colors duration-500 w-full">
            ...
            >
              <div 
                className={`p-4 rounded-2xl border transition-all duration-500 group-hover:scale-105 group-hover:-translate-y-0.5 ${
                  isDark 
                    ? 'bg-white/[0.01] border-white/5 group-hover:border-sky-500/30 group-hover:bg-sky-600/5' 
                    : 'bg-white border-slate-200 group-hover:border-sky-500/30 group-hover:bg-sky-50'
                }`}
              >
                {tech.icon}
              </div>
              
              <span className={`text-lg font-bold font-headline tracking-tight transition-colors duration-300 group-hover:text-sky-500 ${

    ...
    return (
        <section className={`py-48 px-8 relative overflow-hidden transition-colors duration-500 ${isDark ? 'bg-black' : 'bg-slate-50'}`}>
            ...
            <div className="max-w-5xl mx-auto text-center relative z-10 space-y-16">
                ...
                >
                    <button className={`w-full sm:w-auto px-12 py-6 rounded-full font-black text-xs uppercase tracking-[0.3em] transition-all duration-500 shadow-2xl ${isDark ? 'bg-white text-black hover:bg-slate-100 shadow-white/5' : 'bg-slate-900 text-white hover:bg-slate-800 shadow-slate-900/20'}`}>
                        Work with us
                    </button>
                    <button className={`w-full sm:w-auto px-12 py-6 rounded-full font-black text-xs uppercase tracking-[0.3em] transition-all duration-500 border ${isDark ? 'bg-transparent border-white/10 text-white hover:bg-white/5' : 'bg-transparent border-slate-200 text-slate-900 hover:bg-slate-50'}`}>
                        Schedule a Call
                    </button>
                </motion.div>
            </div>
